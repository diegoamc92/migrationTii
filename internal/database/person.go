...
func InsertPersonData(db *sql.Tx, ctx MigrationContext) error {
	...
	WHERE t.RAMO = ? AND t.NPOLIZA = ? AND t.RUT IS NOT NULL -- Filtrar por contexto
	...
	if err != nil {
		return fmt.Errorf("error insertando en PERSON para RAMO %s, NPOLIZA %s: %v", ctx.Ramo, ctx.Npoliza, err)
	}
	fmt.Printf("Datos insertados o actualizados en PERSON correctamente para RAMO %s, NPOLIZA %s.\n", ctx.Ramo, ctx.Npoliza)
	log.Println(query)
	return nil
}
