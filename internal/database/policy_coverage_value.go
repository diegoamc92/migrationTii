...
func InsertPolicyCoverageValue(db *sql.Tx, ctx *MigrationContext) error {
	query := `
    ...
    WHERE p.RAMO = ? AND p.NPOLIZA = ?                      -- Filtro para pólizas específicas
	...
	log.Printf("Datos insertados correctamente en POLICY_COVERAGE_VALUE para POLICY_ID: %s-%s", ctx.Ramo, ctx.Npolori)
	return nil
}
